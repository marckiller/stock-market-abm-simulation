...
class TestOrderMarket(unittest.TestCase):
    ...
    def test_order_market_execute_full(self):
        order = OrderMarket(quantity=15, side=OrderSide.SELL, time=1100)
        order.execute(15)
        self.assertEqual(order.quantity, 0)
        self.assertEqual(order.status, OrderStatus.FILLED)

    def test_order_market_execute_over_quantity(self):
        order = OrderMarket(quantity=15, side=OrderSide.SELL, time=1150)
        order.execute(20)
        self.assertEqual(order.quantity, 0)
        self.assertEqual(order.status, OrderStatus.FILLED)

    def test_order_market_execute_canceled(self):
        order = OrderMarket(quantity=15, side=OrderSide.SELL, time=1200)
        order.cancel()
        order.execute(5)
        self.assertEqual(order.quantity, 15)
    ...
    def test_order_market_execute_expired(self):
        order = OrderMarket(quantity=15, side=OrderSide.SELL, time=1250)
    ...
    def test_order_market_cancel_open(self):
        order = OrderMarket(quantity=15, side=OrderSide.SELL, time=1300)
        order.cancel()
        self.assertEqual(order.status, OrderStatus.CANCELED)

    def test_order_market_cancel_filled(self):
        order = OrderMarket(quantity=15, side=OrderSide.SELL, time=1350)
        order.execute(15)
        order.cancel()
        self.assertEqual(order.status, OrderStatus.FILLED)

    def test_order_market_cancel_already_canceled(self):
        order = OrderMarket(quantity=15, side=OrderSide.SELL, time=1400)
        order.cancel()
        order.cancel()
        self.assertEqual(order.status, OrderStatus.CANCELED)

    def test_order_market_is_active_open(self):
        order = OrderMarket(quantity=15, side=OrderSide.SELL, time=1450)
        self.assertTrue(order.is_active())

    def test_order_market_is_active_filled(self):
        order = OrderMarket(quantity=15, side=OrderSide.SELL, time=1500)
        order.execute(15)
        self.assertFalse(order.is_active())

    def test_order_market_is_active_canceled(self):
        order = OrderMarket(quantity=15, side=OrderSide.SELL, time=1550)
        order.cancel()
        self.assertFalse(order.is_active())

    def test_order_market_is_filled_true(self):
        order = OrderMarket(quantity=15, side=OrderSide.SELL, time=1600)
        order.execute(15)
        self.assertTrue(order.is_filled())

    def test_order_market_is_filled_false_open(self):
        order = OrderMarket(quantity=15, side=OrderSide.SELL, time=1650)
        self.assertFalse(order.is_filled())

    def test_order_market_is_filled_false_canceled(self):
        order = OrderMarket(quantity=15, side=OrderSide.SELL, time=1700)
        order.cancel()
        self.assertFalse(order.is_filled())

    def test_order_market_id_increment(self):
        order1 = OrderMarket(quantity=15, side=OrderSide.SELL, time=1750)
        order2 = OrderMarket(quantity=10, side=OrderSide.BUY, time=1800)
        order3 = OrderMarket(quantity=5, side=OrderSide.SELL, time=1850)
    ...
    def test_order_market_id_with_manual_id(self):
        ...
        order2 = OrderMarket(quantity=10, side=OrderSide.BUY, time=1950)
        order3 = OrderMarket(quantity=5, side=OrderSide.SELL, time=2000)
